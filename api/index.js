...
app.post('/api/orders', async (req, res) => {
  ...
  if (currentTotal + incomingQty > max) {
    return res.status(409).json({ error: `Capacité dépassée pour le créneau ${req.body.slot}` });
  }
  const order = { id: Date.now(), date: new Date().toLocaleString('fr-FR'), name: req.body.name, phone: req.body.phone, commande: req.body.commande, slot: req.body.slot, type: 'emporter', horaire: req.body.horaire || '', status: 'Nouveau' };
...
app.get('/api/slots', async (req, res) => {
  ...
  function parseTime(str) { const clean = str.replace(/[^\d:]/g, ''); const [h, m] = clean.split(':').map(Number); return h * 60 + (m || 0); }
  function formatSlot(minutes) { return `${Math.floor(minutes / 60)}h${(minutes % 60) ? (minutes % 60).toString().padStart(2, '0') : ''}`; }
  const currentMinutes = now.getHours() * 60 + now.getMinutes();
  const slots = [];
  function splitHours(v) { const parts = v.split('–').length > 1 ? v.split('–') : v.split(' - '); return parts.map(s => parseTime(s.trim())); }
  ...
  function countPizzas(commande) { return (commande.match(/(\d+)\s*x?/gi) || []).reduce((sum, m) => sum + parseInt(m), 0) || 1; }
  for (const [start, end] of ranges) {
    for (let t = start; t < end; t += 15) {
      ...
      slots.push({ value: slot, remaining: Math.max(0, max - totalPizzas), max, disponible: totalPizzas < max });
    }
  }
...
app.get('/api/slotSettings', async (req, res) => {
  ...
  data.forEach(s => { if (s.slot_time) grouped[s.jour][s.slot_time] = s.max_pizzas; });
  res.json(grouped);
});
...
app.put('/api/slotSettings', requireAuth, async (req, res) => {
  ...
  for (const ins of inserts) {
    ...
    if (existingRow) { await req.supabase.from('slot_settings').update(ins).eq('id', existingRow.id); }
    else { await req.supabase.from('slot_settings').insert(ins); }
  }
  res.json({ ok: true });
});
...
app.put('/api/settings', requireAuth, async (req, res) => {
  for (const [key, value] of Object.entries(req.body)) {
    ...
    if (existing) { await req.supabase.from('settings').update({ value }).eq('id', existing.id); }
    else { await req.supabase.from('settings').insert({ key, value }); }
  }
  res.json(req.body);
});
...
app.put('/api/hours', requireAuth, async (req, res) => {
  for (const [jour, h] of Object.entries(req.body)) {
    ...
    if (existing) { await req.supabase.from('hours').update({ midi: h.midi, soir: h.soir, ferme: h.ferme }).eq('id', existing.id); }
    else { await req.supabase.from('hours').insert({ jour, midi: h.midi, soir: h.soir, ferme: h.ferme }); }
  }
  res.json(req.body);
});
